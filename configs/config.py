...
def get_config(project = '', mode = '', config_ = ''):
    ...
    ## Naive Decay
    config.LRS = 'LD' # LD
    config.total_itr = 6000000
    config.decay_period = [5000000, 5500000]
    ...
    # data dir
    config.data_offset = 'F:\\DPDD-SL'
    config.c_path = os.path.join(config.data_offset, 'DPDD-SL\\train_c')
    ...
    # data path
    config.VAL = edict()
    config.VAL.c_path = os.path.join(config.data_offset, 'DPDD-SL\\val_c')
...
def set_eval_path(config, data):
    if data == 'DPDD-SL' :
        config.EVAL.c_path = os.path.join(config.data_offset, 'DPDD-SL\\test_c')
        ...
        config.EVAL.c_path = os.path.join(config.data_offset, 'COMPER\\test_c')

        # child paths
        config.EVAL.input_path = 'source'
        config.EVAL.gt_path = 'target'
    elif data == 'test':
        config.EVAL.c_path = os.path.join(config.data_offset, 'test')
