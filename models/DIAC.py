...
from models.LaKD_block import  DLKD_block


def DIAC(feat_in, F, N, c, k, is_act_last = True):
...
def SAC(feat_in, kernel1, kernel2, ksize):
    ...
    sfLaKD = DLKD_block(in_channels=channels,out_channel=channels)
    ...
    feat_in = torch.sum(torch.mul(feat_in, kernel2), -1)
