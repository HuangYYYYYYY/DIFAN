...
from data_loader.datasets_aug import datasets
...
class Model(baseModel):
    ...
    def _set_results(self, inputs, outs, errs, lr, norm=1):
        ...
        if self.config.save_sample:
            ...
            if 'R' in self.config.mode or 'DIFAN' in self.config.mode or 'C_only' in self.config.mode:
    ...
    def _get_results(self, C, GT, is_train):

        if self.config.is_amp:
            with torch.cuda.amp.autocast():
                outs = self.network(C,  GT, is_train=is_train)
        else:
            outs = self.network(C, GT, is_train=is_train)
        ...
        if self.config.is_train:
            ...
            if is_train:
                ...
                if 'P' in self.config.mode or 'DIFAN' in self.config.mode :
                    errs['BPR'] = self.MSE(outs['BPR'], outs['PMP'])
                    errs['total'] = errs['total'] + errs['BPR']
            else:
                errs['psnr'] = get_psnr2(outs['result'], GT)
                errs['ssim'] = get_ssim(outs['result'], GT)
                dist = self.LPIPS.forward(outs['result'] * 2. - 1., GT * 2. - 1.) #imge range [-1, 1]
                errs['LPIPS'] = torch.mean(dist) # flow log
    ...
    def iteration(self, inputs, epoch, max_epoch, is_train):
        ...
        errs, outs = self._get_results(C, GT, is_train)
        if self.config.is_amp:
...
class DeblurNet(nn.Module):
    def __init__(self, config, lib):
        ...
        if self.config.is_train and ('R' in config.mode or 'DIFAN' in config.mode or 'C_only' in self.config.mode):
    ...
    def init(self):
        ...
        self.Network.init_F()
        if self.config.is_train and ('R' in self.config.mode or 'DIFAN' in self.config.mode or 'C_only' in self.config.mode):
    ...
    def forward(self, C, GT=None, is_train=False):
        ...
        outs = self.Network(C, GT, is_train)

        if is_train and ('R' in self.config.mode or 'DIFAN' in self.config.mode or 'C_only' in self.config.mode):
