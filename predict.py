...
from configs.config_DIFAN import get_config
...
class Predictor(cog.Predictor):
    def setup(self):
        ...
        self.config = get_config('DIFAN_2023', 'DIFAN', 'config_DIFAN')
        self.config.network = 'DIFAN'
        model = create_model(self.config)
        self.network = model.get_network().eval()
        self.network = self.network.to(self.device)

        ckpt_manager = CKPT_Manager(root_dir='', model_name='DIFAN')
        load_state, ckpt_name = ckpt_manager.load_ckpt(self.network, abs_name = './ckpt/DIFAN.pytorch')
